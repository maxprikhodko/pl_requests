...
prompt ----------------------------------------------;
prompt -- Creating public synonyms for PL_REQUESTS --;
prompt ----------------------------------------------;
...
begin
  ...
  loop
    ...
    then
      begin
        execute immediate 'create public synonym ' || l_objects(i) || ' for ' || c_schema || '.' || l_objects(i);
      exception
        when OTHERS then
